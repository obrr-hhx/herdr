...
If a machine stops responding during a switch, Herdr retires that connection after the presentation deadline and continues with a healthy machine. It preserves your latest pending selection if recovery fails; pane input resumes only after the selected machine supplies a fresh, coherent screen. Temporary SSH proxy handshake failures retry automatically. With `remote.manage_ssh_config` enabled, background discovery and the bridge share a private SSH connection to reduce repeated connection setup.
