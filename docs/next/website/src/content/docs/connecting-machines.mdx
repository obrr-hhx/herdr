...
Switching to another machine immediately revokes the old machine’s local presentation and starts activating the target. Herdr releases the old remote surface in the background; waiting for its acknowledgement or cleaning up a timed-out connection does not delay the target. Input resumes after the target supplies a coherent screen. Temporary SSH proxy handshake failures retry automatically. With `remote.manage_ssh_config` enabled, background discovery and the bridge share a private SSH connection.

The sidebar keeps its local mouse capture even when a remote endpoint disables mouse reporting. Machine clicks are retained locally and take priority over queued transport events, so a busy remote connection cannot drop the latest selection.
